...
func readJpegOrPng(f *multipart.File, b *bytes.Buffer) (err error) {
	if w, err := io.Copy(b, f); err != nil {
		return
	} else if w > MaxBytes {
		err = errors.New("File size is over max size")
	}
	return
}

func readGif(f *multipart.File, b *bytes.Buffer) (err error) {
